...
export default function AdminSessionsPage() {
  ...
  const { data, error, mutate, isLoading } = useSWR<SessionsResponse>(
    swrKey,
    fetcher,
    {
  ...
  const filterControls = (
    <div className="flex flex-wrap gap-2 items-end mb-2 w-full justify-between bg-card/50 backdrop-blur-sm border border-border/50 rounded-lg p-4">
      ...
      <div className="text-sm text-foreground/60">{total} sesiones activas</div>
    </div>
  );
  ...
  if (error)
    return <div className="p-6 text-destructive">Falló al cargar sesiones</div>;
